...
#ifdef LOCALHOST
				usleep(1000);
				ios[j] = new NetIO(IP[j], port+4*(i*nP+j), true);
#else
				usleep(1000);
				ios[j] = new NetIO(IP[j], port+4*(i), true);
#endif
				ios[j]->set_nodelay();	

#ifdef LOCALHOST
				usleep(1000);
				ios2[j] = new NetIO(nullptr, port+4*(i*nP+j)+1, true);
#else
				usleep(1000);
				ios2[j] = new NetIO(nullptr, port+4*(j)+1, true);
#endif
				ios2[j]->set_nodelay();	
			} else if(j == party) {
#ifdef LOCALHOST
				usleep(1000);
				ios[i] = new NetIO(nullptr, port+4*(i*nP+j), true);
#else
				usleep(1000);
				ios[i] = new NetIO(nullptr, port+4*(i), true);
#endif
				ios[i]->set_nodelay();	

#ifdef LOCALHOST
				usleep(1000);
				ios2[i] = new NetIO(IP[i], port+4*(i*nP+j)+1, true);
#else
				usleep(1000);
				ios2[i] = new NetIO(IP[i], port+4*(j)+1, true);
#endif
				ios2[i]->set_nodelay();	
			}
		}
